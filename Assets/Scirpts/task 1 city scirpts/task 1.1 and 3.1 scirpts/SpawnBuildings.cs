...
public class SpawnBuildings : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        while (count < Mathf.Min(maxAmountOfBuildings, amountOfChildGameObjects))
        {
           ...
           while (availableNumbers.Contains(pickedIndex)) // this basically prevents houses from spawning each other as generating a random number may be already generated
            {
                pickedIndex = Random.Range(0, amountOfChildGameObjects);

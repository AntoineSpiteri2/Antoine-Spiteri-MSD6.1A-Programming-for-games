    ...
    void CreateCube()
    {
        ...
        foreach (int[] triangle in cubeTriangles)
        {
            ...
            meshBuilder.BuildTriangle(vertices[triangle[3]], vertices[triangle[4]], vertices[triangle[5]], 0);
                ...
                // After creating the mesh, assign it to the mesh filter
        Mesh mesh = meshBuilder.CreateMesh();
        meshFilter.mesh = mesh;


        // Add or update the Mesh Collider
        MeshCollider meshCollider = gameObject.GetComponent<MeshCollider>();
        if (meshCollider == null)
        {
            meshCollider = gameObject.AddComponent<MeshCollider>();
        }
        meshCollider.sharedMesh = mesh; // Assign the current mesh to the collider
